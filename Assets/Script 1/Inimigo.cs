...
public class Inimigo : MonoBehaviour
{
        ...
        if (!inimtamorto)
        {
            if (MinhaPosicao.Length != 0)
            {
                ...
                else
                {
                    ...
                    transform.position = new Vector3(transform.position.x, transform.position.y,Mathf.Clamp(transform.position.z ,- 9f,-1.5f));
                    ...
                    if (distanciaalvo.magnitude < 1.8f)
                    {
    ...
    {
        if (olhad)
            inimrb.AddForce(new Vector3(-25, 20, 0), ForceMode.Impulse);
